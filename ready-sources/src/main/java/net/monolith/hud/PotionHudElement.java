...
public class PotionHudElement extends HudElement {
   ...
   public void render(DrawContext context, float tickDelta) {
      ...
      if (mc.player != null) {
         ...
         effects.sort(Comparator.comparing(effectx -> effectx.getEffectType().value().getName().getString().toLowerCase(Locale.ROOT)));
         ...
         if (!(this.alphaAnim < 0.02F) || visible) {
            ...
            float targetWidth = Math.max(132.0F, this.maxWidth(mc, effects, isChatOpen));
            ...
            int bg = HudStyle.withAlpha(0xFF121827, Math.min(150, alpha));
            HudStyle.panel(renderer, (float)this.x, (float)this.y, (float)this.width, (float)this.height, 9.0F, alpha);
            HudStyle.header(renderer, mc, "P", "Potions", this.x + 4, this.y + 3, this.width - 8, alpha);
            int rowY = this.y + 29;
   ...
   private void renderPotionRow(
      ...
      context.drawTexture(RenderLayer::getGuiTextured, icon, this.x + 10, rowY - 1, 0.0F, 0.0F, 9, 9, 9, 9);
      int durationWidth = renderer.textWidth(mc.textRenderer, duration);
      int levelX = this.x + 26 + renderer.textWidth(mc.textRenderer, name) + 4;
      String visibleName = mc.textRenderer.trimToWidth(name, Math.max(20, this.width - durationWidth - renderer.textWidth(mc.textRenderer, level) - 42));
      renderer.text(mc.textRenderer, visibleName, this.x + 26, rowY, textColor, false);
      renderer.text(mc.textRenderer, level, levelX, rowY, levelColor, false);
      renderer.text(mc.textRenderer, duration, this.x + this.width - durationWidth - 11, rowY, textColor, false);
   }

   private float maxWidth(MinecraftClient mc, List<StatusEffectInstance> effects, boolean edit) {
      float max = (float)(mc.textRenderer.getWidth("Potions") + 58);
      if (effects.isEmpty() && edit) {
         return Math.max(max, (float)(mc.textRenderer.getWidth("Preview") + mc.textRenderer.getWidth("10") + mc.textRenderer.getWidth("**:**") + 54));
      } else {
         for (StatusEffectInstance effect : effects) {
            String name = effect.getEffectType().value().getName().getString();
            String level = String.valueOf(effect.getAmplifier() + 1);
            String duration = this.formatDuration(effect.getDuration() / 20);
            max = Math.max(max, (float)(mc.textRenderer.getWidth(name) + mc.textRenderer.getWidth(level) + mc.textRenderer.getWidth(duration) + 58));
         }
